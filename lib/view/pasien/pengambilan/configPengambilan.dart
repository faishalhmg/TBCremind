...
class ModifyAlarmPengambilanScreenArg {
  final PengambilanDataModel alarm;
  final int index;

  ModifyAlarmPengambilanScreenArg(this.alarm, this.index);
}

class ConfigPengambilan extends StatefulWidget {
  final ModifyAlarmPengambilanScreenArg? arg;
  const ConfigPengambilan({super.key, this.arg});

  @override
  State<ConfigPengambilan> createState() => _ConfigPengambilanState();
}

class _ConfigPengambilanState extends State<ConfigPengambilan> {
  TextEditingController _lokasiTextController = TextEditingController();
  late PengambilanDataModel alarm = widget.arg?.alarm ??
      PengambilanDataModel(
        time: DateTime.now(),
        date1: DateTime.now(),
        date2: DateTime.now(),
        lokasi: '',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text((_editing ? 'Edit' : 'Tambah') + ' Pengambilan Obat',
            ...
            child: GestureDetector(
              onTap: () async {
                if (alarm.lokasi.isNotEmpty) {
                  final model = context.read<PengambilanModel>();
                  context.goNamed('pengambilanObat');
                  _editing
                      ? await model.updatePengambilan(alarm, widget.arg!.index)
                      : await model.addPengambilan(alarm);
                } else {
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Expanded(child: Text('Lokasi ambil')),
                              Expanded(
                                child: Container(),
                              ),
                              Expanded(
